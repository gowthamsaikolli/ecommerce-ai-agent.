...
async def ask_endpoint(input: QuestionInput):
    ...
    prompt = (
        ...
        "Answer: SELECT SUM(total_sales) FROM total_sales;"
        "Question: Which product had the highest CPC?\n"
        "Answer: SELECT item_id FROM ad_sales WHERE clicks > 0 GROUP BY item_id ORDER BY SUM(ad_spend)/SUM(clicks) DESC LIMIT 1;"
